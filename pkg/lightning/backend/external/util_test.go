...
func TestGetReadRangeFromProps(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []uint64{10, 20}, got[0][0])
	require.Equal(t, []uint64{36, 46}, got[0][1])
	...
	require.Equal(t, []uint64{10, 20}, got[0][0])
	require.Equal(t, []uint64{36, 46}, got[0][1])
	require.Equal(t, []uint64{10, 20}, got[1][0])
	require.Equal(t, []uint64{36, 46}, got[1][1])

	// key exactly on a prop boundary
	got, err = getReadRangeFromProps(ctx, [][]byte{[]byte("key3")}, paths, store)
	require.NoError(t, err)
	require.Equal(t, []uint64{30, 20}, got[0][0])
	require.Equal(t, []uint64{56, 46}, got[0][1])
	...
	require.Equal(t, []uint64{10, 20}, got[0][0])
	require.Equal(t, []uint64{36, 46}, got[0][1])
	require.Equal(t, []uint64{30, 20}, got[1][0])
	require.Equal(t, []uint64{56, 46}, got[1][1])

	// key below all props
	got, err = getReadRangeFromProps(ctx, [][]byte{[]byte("key0")}, paths, store)
	require.NoError(t, err)
	require.Equal(t, []uint64{0, 0}, got[0][0])
	require.Equal(t, []uint64{0, 0}, got[0][1])

	// key exactly on first prop
	got, err = getReadRangeFromProps(ctx, [][]byte{[]byte("key1")}, paths, store)
	require.NoError(t, err)
	require.Equal(t, []uint64{10, 0}, got[0][0])
	require.Equal(t, []uint64{36, 0}, got[0][1])
	...
	require.Equal(t, []uint64{0, 0}, got[0][0])
	require.Equal(t, []uint64{0, 0}, got[0][1])
	require.Equal(t, []uint64{10, 0}, got[1][0])
	require.Equal(t, []uint64{36, 0}, got[1][1])

	// key above all props
	got, err = getReadRangeFromProps(ctx, [][]byte{[]byte("key999")}, paths, store)
	require.NoError(t, err)
	require.Equal(t, []uint64{50, 40}, got[0][0])
	require.Equal(t, []uint64{76, 66}, got[0][1])
	...
	require.Equal(t, []uint64{50, 40}, got[0][0])
	require.Equal(t, []uint64{76, 66}, got[0][1])
	require.Equal(t, []uint64{50, 40}, got[1][0])
	require.Equal(t, []uint64{76, 66}, got[1][1])
	...
	require.NoError(t, err)
	require.Equal(t, []uint64{30, 20, 0}, got[0][0])
	require.Equal(t, []uint64{56, 46, 0}, got[0][1])
}
